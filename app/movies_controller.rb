...
def can_be_instantiated_and_then_saved
  movie = Movie.new         #creates a new entry in the database with nil values as attributes
  movie.title = "This is a title."    #sets an attribute in the instance
  movie.save                          #saves teh instances attribute into the db table
end
...
def can_be_created_with_a_hash_of_attributes
  ...
  movie = Movie.create(attributes)     #creates a new entry into the db table with attributes that are predefined in the spec file
end
...
def can_find_the_first_item_from_the_database_using_id
  Movie.find_by(1).title   #find the 1 db entry and return title
end
