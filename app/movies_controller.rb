...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  Movie.all.find {|m| m.title == "Title"
                      m.release_date == 2000
                      m.director == "Me" }
end
...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.all.where(:release_date => [2003, 2004]).reverse
end

def can_be_found_updated_and_saved
  # Updtate the title "Awesome Flick" to "Even Awesomer Flick"
  Movie.create(title: "Awesome Flick")
  m = Movie.all.find {|m| m.title == "Awesome Flick"}
...
def can_destroy_a_single_item
  ...
  m = Movie.find { |m| m.title == "That One Where the Guy Kicks Another Guy Once" }
  m.destroy
end
