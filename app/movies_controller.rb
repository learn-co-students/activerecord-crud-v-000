...
def can_be_instantiated_and_then_saved
  movie = Movie.new #instantiate new movie instance
  movie.title = "This is a title."
  movie.save #commit record to db
end
...
def can_get_the_first_item_in_the_database
  Movie.first.title
  #alternative using #find and id >> Movie.find(1).title 
end
...
def can_get_size_of_the_database
  Movie.count
  #alternative >> Movie.all.size
end

def can_find_the_first_item_from_the_database_using_id
  Movie.find(1).title
  #alternative >> Movie.first.title 
end

#pull request to update this method name
