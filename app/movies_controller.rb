...
def can_find_the_first_item_from_the_database_using_id
  Movie.all.find {|movies| movies.id == 1 }.title
end
...
def can_update_multiple_items_at_once
  ...
  Movie.all.each {|movies| movies.update(title: "A Movie")}
...
def can_destroy_a_single_item
  ...
  id = Movie.all.find_by(title: "That One Where the Guy Kicks Another Guy Once").id
  binding.pry
