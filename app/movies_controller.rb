...
def can_find_using_where_clause_and_be_sorted
  ...
  Movie.all.each do |mov|
    result << mov if mov.release_date > 2002
  end
  result.reverse
end
...
def can_be_found_updated_and_saved
  ...
  j=Movie.find_by(title: "Awesome Flick")
  j.title = "Even Awesomer Flick"
  j.save
end
...
def can_update_multiple_items_at_once
  ...
  Movie.all.each {|mov| Movie.update(mov.id, :title => "A Movie")}
end
...
def can_destroy_all_items_at_once
  ...
  Movie.all.each {|mov| Movie.destroy(mov.id)}
end
